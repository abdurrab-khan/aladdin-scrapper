...
export class GroupedScreenshotHandler extends BaseScreenshotHandler {
  async execute(
    ...
    if (!priceDetails) {
      throw new Error(`Price details are required for grouped screenshot for: ${website}`);
    }
    ...
    try {
      ...
      for (let i = 0; i < productCards.length && i < 4; i++) {
        ...
        const isValid = await this.provider.isValidProduct(priceElem, discountElem, priceDetails);
        if (!isValid) {
          if (i >= 2) break;
      ...
      const path = `./src/product_images/grouped_${id}_image.png`;
      ...
      throw new Error((error as Error)?.message ?? "Error taking grouped screenshot");
    } finally {
  ...
  private async calculateClip(elems: ElementHandle<Element>[]): Promise<ScreenshotClip> {
    let x = 0;
    let y = 0;
    let maxHeight = 0;
    let maxWidth = 0;
