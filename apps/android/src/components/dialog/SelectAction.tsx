...
const SelectAction: React.FC = () => {
  const { productSelectionData, setProductSelectionData, clearSelection } =
    useProductStore();
  ...
  const productsWithNoAffiliate = useMemo(() => {
    return Array.from(productSelectionData.entries())
      .filter(([_, value]) => !value.hasAffiliateLink)
      .map(([id]) => id);
  }, [productSelectionData]);

  const isProductsGrouped = useMemo(() => {
    return Array.from(productSelectionData.values()).some(
      (value) => value.isGrouped,
    );
  }, [productSelectionData]);
  ...
  const handlePageRedirect = () => {
    ...
    if (isProductsGrouped && productSelectionData.size > 1) {
      ToastAndroid.show(
        "Grouped products cannot be shared. Please send as single.",
        ToastAndroid.SHORT,
      );
      return;
    }
    ...
    if (productsWithNoAffiliate.length > 0) {
      ToastAndroid.show(
        "Please add affiliate link to all selected products",
        ToastAndroid.LONG,
      );
    }
