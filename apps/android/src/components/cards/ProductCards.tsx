...
function ProductCards({
  ...
  const [showAffiliateDialog, setShowAffiliateDialog] =
  ...
  const shareGradientColors = React.useMemo<
  ...
  const cardImage = React.useMemo(() => {
    const cardImg =
      images.find((img) => img.image_type === "Card") || images[0];
...
export default React.memo(ProductCards, (prevProps, nextProps) => {
  ...
  if (
    prevProps.hasSelected !== nextProps.hasSelected ||
