...
export class FullScreenshotHandler extends BaseScreenshotHandler {
  async execute(id: string, url: string, website: Website): Promise<string> {
    ...
    try {
      ...
      if (!mainElement) throw new Error(`Could not find main element for ${website}`);
      ...
      if (!mainBounding) throw new Error(`Could not get bounding box for ${website}`);
      ...
      throw new Error((error as Error)?.message ?? "Error taking full screenshot");
    } finally {
